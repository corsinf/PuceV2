...
class localizacionM
{
	...
	function localizacion_custodio($query,$custodio,$ini=0,$fin=25)
	{
		// $sql = "SELECT ID_MARCA,CODIGO,DESCRIPCION FROM localizacion ";
		$sql1='SELECT DISTINCT(LOCATION) as "id" FROM PLANTILLA_MASIVA WHERE PERSON_NO IN ('.$custodio.')';
		// print_r($sql1);die();
		$datos = $this->db->datos($sql1);
		$id='';
		foreach ($datos as $key => $value) {
			$id.="'".$value['id']."',";			
		}
		$id = substr($id,0,-1);
		$sql = "SELECT ID_LOCATION,CENTRO,EMPLAZAMIENTO,DENOMINACION FROM LOCATION WHERE ESTADO='A' and DENOMINACION+''+EMPLAZAMIENTO LIKE '%".$query."%' ";

		if($id!='')
		{
		  $sql.=" AND ID_LOCATION in (".$id.") ";
		}
		$sql.=" ORDER BY ID_LOCATION DESC OFFSET ".$ini." ROWS FETCH NEXT ".$fin." ROWS ONLY;";
		 // print_r($sql);die();
		$datos = $this->db->datos($sql);
		return $datos;
	}
