...
class detalle_articuloM
{
	...
	function cargar_detalle_activo($asset)
	{
		$sql="SELECT P.id_plantilla as 'id_A',A.ID_ASSET as 'id_AS',DESCRIPT as 'nom',DESCRIPT2 as 'des',TAG_SERIE as 'tag_s',TAG_UNIQUE as 'rfid',TAG_ANT as 'ant',COMPANYCODE,MODELO,SERIE,FECHA_INV_DATE as 'fecha',P.LOCATION as 'id_loc',L.DENOMINACION as 'DENOMINACION',P.PERSON_NO as 'id_cus',PERSON_NOM,MARCAS.ID_MARCA as 'mar',MARCAS.DESCRIPCION as 'marca',ESTADO.ID_ESTADO as 'est',GENERO.ID_GENERO as 'gen',GENERO.DESCRIPCION as 'genero',COLORES.ID_COLORES as 'col',COLORES.DESCRIPCION as 'color',PROYECTO.ID_PROYECTO as 'idpro',PROYECTO.denominacion as 'proyecto',OBSERVACION ,IMAGEN,QUANTITY,BASE_UOM,ORIG_ASSET,ORIG_ACQ_YR,ORIG_VALUE,CARACTERISTICA,EVALGROUP5,BAJAS,PATRIMONIALES,TERCEROS,ASSETSUPNO,COMPANYCODE,P.FAMILIA as 'IDF' FROM PLANTILLA_MASIVA P 
		LEFT JOIN ASSET A ON P.ID_ASSET = A.ID_ASSET 
		LEFT JOIN LOCATION L ON P.LOCATION = L.ID_LOCATION
		LEFT JOIN PERSON_NO PE ON P.PERSON_NO = PE.ID_PERSON 
		LEFT JOIN MARCAS ON P.EVALGROUP1 = MARCAS.ID_MARCA 
		...
		LEFT JOIN COLORES ON P.EVALGROUP4 = COLORES.ID_COLORES
		LEFT JOIN PROYECTO ON P.EVALGROUP5 = PROYECTO.ID_PROYECTO
		WHERE A.TAG_SERIE = '".$asset."' OR A.TAG_UNIQUE = '".$asset."' OR A.TAG_ANT = '".$asset."'";
		// print_r($sql);die();
		$datos = $this->db->datos($sql);
		return $datos;
	}
