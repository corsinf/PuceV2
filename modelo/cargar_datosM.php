...
class cargar_datosM
{
	...
	function log_activo($fecha=false,$fecha2=false,$intento=false,$accion=False,$estado=false,$encargado = false)
	{

		$sql="SELECT * FROM log_activos WHERE 1=1 ";
		if($fecha && $fecha2==false)
		{
			$sql.=" AND fecha = '".$fecha."'";
		}else if($fecha!=false && $fecha2!=false)
		{
			$sql.=" AND fecha between '".$fecha."' and '".$fecha2."'";
		}else if($fecha==false && $fecha2!=false)
		{
			$sql.=" AND fecha = '".$fecha."'";
		}
		...
		if($encargado)
		{
			$sql.=" AND usuario like '%".$encargado."%'";
		}
