...
class modulos_paginasM
{
	...
	function paginas($query=false,$modulo=false)
	{
		...
		LEFT JOIN MODULOS M ON P.id_modulo = M.id_modulo WHERE AC.Ver = 1 AND subpagina =0 AND AC.id_tipo_usu = '".$_SESSION['INICIO']['PERFIL']."' ";
		if($query)
		{
		...
		$sql.=' ORDER BY nombre_pagina ASC';
	...
	{
		$sql = "DELETE FROM PAGINAS WHERE id_paginas = '".$id."'";
		return $this->db->sql_string($sql);
