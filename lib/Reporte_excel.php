...
class Reporte_excel
{
	...
	function Reporte_sap($query,$loc,$cus,$desde,$hasta)
	{
		...
		$cambios = $this->articulos->cambios_unicos($desde,$hasta);
		$datos =array();
		// print_r($cambios);die();
		foreach ($cambios as $key => $value) {
			if($value['id_plantilla']!=0)
			{
				$datos[] = $this->articulos->lista_articulos_sap_codigos($query,$loc,$cus,false,$value['id_plantilla'],false);
			}
    }
     ...
     // print_r($datos);die();
	  ...
	  $sheet->setCellValue('AH2','BWASL');
	  $sheet->setCellValue('AI2','NOTA_INVENTARIO');
	  ...
	  $sheet->setCellValue('AH1','CLASE DE MOVIMIENTO');
	  $sheet->setCellValue('AI1','NOTA INVENTARIO');
...
// $datos = $datos[0];
		...
		// $fecha = $value['FECHA_INV_DATE']->format('Y-m-d');
			...
			{
				$fecha =$value['FECHA_INV_DATE']->format('Ymd');
				//$fecha = new DateTime($fecha);
				// $fecha = \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($fecha);
				//$fecha = str_replace('-','',substr(string, start))
			}
			$fechaC='';
...
// print_r($value);die();
		...
		$sheet->setCellValue('AI'.$count,$value['NOTA_INVE']);
		$count = $count+1;
	  }
	  ...
	  $nombre_rp = 'Reporte_activos_'.str_replace(" ","_", date('Ymd His')).'.xlsx';
		$write->save($nombre_rp);
		echo "<meta http-equiv='refresh' content='0;url=$nombre_rp'/>";
		exit;
...
set_time_limit(0);
    ...
    function reporte_actual($parametros){
		...
		$multiple = 0;
		if(isset($parametros['multiple']) && $parametros['multiple']=='true' || $parametros['multiple']==1 && $query!='')
		{
			$multiple = 1;
		}
...
*/
   function reporte_log($parametros){
    ...
    $salida = '<table class="table table-striped" border="1">
	  ...
	  <tbody>';
	  $datos = $this->carga_datos->log_activo($parametros['txt_fecha'],$parametros['txt_fecha2'],$parametros['txt_intento'],$parametros['txt_accion'],$parametros['rbl_estado'],$parametros['txt_encargado']);
	  // print_r($datos);die();
	  foreach ($datos as $key => $value) {
		// $fecha = $value['FECHA_INV_DATE']->format('Y-m-d');
		  $fecha='';
