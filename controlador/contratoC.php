...
class contratoC 
{
    	...
    	foreach ($datos as $key => $value) {
    		$tr.='<tr>
	    		...
	    		if($_SESSION['INICIO']['ELIMINAR']==1){
	    		 $tr.='<td><button class="btn btn-sm btn-danger" onclick="eliminar_art(\''.$value['id'].'\')"><i class="fa fa fa-trash"></i></button></td>';
	    		}
	    		$tr.'</tr>';
    	}
    	return $tr;
	...
	function historial_siniestro($parametros)
	{
		...
		$pendiente = 0;
		foreach ($datos as $key => $value) {
			$es = '<button disable="" class="btn btn-sm"><i class="fa fa-lock"></i></button>';
			if($value['estado_proceso']==0)
			{
				$es = '<button class="btn-info btn btn-sm" onclick="cerrar_siniestro('.$value['id_deterioro'].')"><i class="fa fa-unlock"></i></button>';
				$pendiente = 1;
			}
			$tr.='<tr>
			<td>'.$es.'</td>
