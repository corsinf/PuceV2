		...
		if(count($sin_modulo)>0)
		{
		  $paginas = $this->modulos->paginas($query=false,$sin_modulo[0]['id']);
			foreach ($paginas as $key => $value) {
				// print_r($value);die();
			$opciones.= '<li class="nav-item">
				            <a href="'.$value['link_pagina'].'" class="nav-link" id="'.$value['id_paginas'].'">
				              '.$value['icono_paginas'].'
				              <p>'.$value['nombre_pagina'].'</p>
				            </a>
				          </li>';
			}
		}
								...
								if(count($paginas)>0)
								{
									 $opciones.='<li class="nav-item">
									            <a href="#" class="nav-link">
									              '.$value['icono'].'
									              <p>'.$value['modulo'].'<i class="fas fa-angle-left right"></i></p>
