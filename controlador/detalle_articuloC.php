...
require_once('../modelo/marcasM.php');
require_once('../modelo/estadoM.php');
require_once('../modelo/proyectosM.php');
require_once('../modelo/custodioM.php');
require_once('../modelo/localizacionM.php');
...
{	
	echo json_encode($controlador->cargar_detalle_activo($_POST['id']));
}
...
class detalle_articuloC
{
	...
	private $custodio;
	private $estado;
	private $contador_movimientos = 0;
	...
	function cargar_datos($id)
	{
		...
		if(count($datos)>0)
		{
			if(!file_exists('//CORS001/xampp/htdocs/img/'.$datos[0]['IMAGEN']))
			{
	...
	{
		$datos = $this->modelo->cargar_detalle_activo($id);
		if(count($datos)>0)
		{
			return $datos;
		}
	...
	function guardar_datos($parametros)
	{
		...
		$bajas=0;$tercero=0;$patrimoniales = 0;
		// print_r($parametros);die();
		...
		if($parametros['fech']!='')
		{
			...
			$datos1[4]['dato']= date('Ymd',strtotime($parametros['fech']));
		}
		...
		if($parametros['bajas']=='true'){$bajas = 1;}
		$datos1[20]['campo']='BAJAS';
		$datos1[20]['dato']=$bajas;

		if($parametros['terceros']=='true'){$tercero = 1;}
		$datos1[21]['campo']='TERCEROS';
		$datos1[21]['dato']=$tercero;

		if($parametros['patrimoniales']=='true'){$patrimoniales = 1;}
		...
		$datos1[26]['campo']='NOTA_INVE';
		$datos1[26]['dato']=$parametros['nota_inv'];
		...
		if($respuesta == 1 and $respuesta1 ==1)
		{
			...
			return array('status'=>1,'movimientos'=>$this->contador_movimientos);
		}else
		{
			return array('status'=>-1,'movimientos'=>0);
		}
	}
	...
	function guardar_datos_patrimoniales($parametros)
	{
		...
		if($parametros['idAr']!='')
		{
			...
			$where1[0]['dato']=$parametros['idAr'];
			$respuesta1 = $this->modelo->update_data('PLANTILLA_MASIVA',$datos1,$where1);
		}else{ $respuesta1 = $this->modelo->guardar('PLANTILLA_MASIVA',$datos1); $movimientos='nuevo ingreso';}
		if($respuesta == 1 and $respuesta1 ==1)
		{
			$texto = $parametros['company'].';'.$parametros['asse'].';0;'.$parametros['desc'].';'.$parametros['des2'].';'.$parametros['mode'].';'.$parametros['seri'].';'.$parametros['rfid'].';'.$parametros['fech'].';'.$parametros['cant'].';'.$parametros['uni'].';'.$loc[0]['EMPLAZAMIENTO'].';'.$loc[0]['DENOMINACION'].';'.$cus[0]['PERSON_NO'].';'.$cus[0]['PERSON_NOM'].';'.$marca[0]['CODIGO'].';'.$est[0]['CODIGO'].';'.$genero[0]['CODIGO'].';'.$color[0]['CODIGO'].';'.$pro[0]['pro'].';'.$parametros['assetno'].';'.$parametros['act'].';'.$parametros['compra'].';'.$parametros['valor'].';'.$parametros['obse'].';'.$parametros['cara'].';fecha_descapitalizacion;'.$parametros['bajas'].';';
	...
	function registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$dato_ant=false,$dato_act=false,$cod_ant=false,$cod_nue=false)
	{
		$this->contador_movimientos++;
		$this->cod_globales->ingresar_movimientos($id,$movimiento,$seccion,$dato_ant,$dato_act,$cod_ant,$cod_nue);
	}
	...
	function comparacion_movimiento($id,$parametros)
	{
		...
		if($parametros['desc']!=$datos[0]['nom'])
		{
			...
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['nom'],$parametros['desc'],'','');
		}
		if($parametros['des2']!=$datos[0]['des'])
		{
			$movimiento=' Se cambio DESCRIPCION 2 de '.$datos[0]['des'].' a '.$parametros['des2'];
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['des'],$parametros['des2'],'','');
		}
		if($parametros['loca']!=$datos[0]['id_loc'])
		{
			...
			// print_r($loc);print_r($locAnt);die();	
			$movimiento=' Se cambio LOCALIZACION de '.$locAnt[0]['DENOMINACION'].' a '.$loc[0]['DENOMINACION'];
			// print_r($movimiento);die();
				$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$locAnt[0]['DENOMINACION'],$loc[0]['DENOMINACION'],$locAnt[0]['EMPLAZAMIENTO'],$loc[0]['EMPLAZAMIENTO']);
		...
		if($parametros['cust']!=$datos[0]['id_cus'])
		{
			...
			$movimiento=' Se cambio CUSTODIO de '.$datos[0]['PERSON_NOM'].' a '.$cus[0]['PERSON_NOM'];	
				$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['PERSON_NOM'],$cus[0]['PERSON_NOM'],$cusAnt[0]['PERSON_NO'],$cus[0]['PERSON_NO']);
		...
		if($parametros['marc']!=$datos[0]['mar'])
		{
			...
			$movimiento=' Se cambio MARCA de '.$datos[0]['marca'].' a '.$marca[0]['DESCRIPCION'];
				$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['marca'],$marca[0]['DESCRIPCION'],$marcaAnt[0]['CODIGO'],$marca[0]['CODIGO']);
		...
		if($parametros['colo']!=$datos[0]['col'])
		{
			...
			$movimiento=' Se cambio COLOR de '.$datos[0]['color'].' a '.$color[0]['DESCRIPCION'];
				$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['color'],$color[0]['DESCRIPCION'],$colorAnt[0]['CODIGO'],$color[0]['CODIGO']);
		...
		if($parametros['gene']!=$datos[0]['gen'])
		{
			...
			$movimiento=' Se cambio GENERO de '.$datos[0]['genero'].' a '.$genero[0]['DESCRIPCION'];
				$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['genero'],$genero[0]['DESCRIPCION'],$generoAnt[0]['CODIGO'],$genero[0]['CODIGO']);
		
		}
		if($parametros['asse']!=$datos[0]['tag_s'])
		{
			$movimiento=' Se cambio ASSET de '.$datos[0]['tag_s'].' a '.$parametros['asse'];
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['tag_s'],$parametros['asse'],'','');
		...
		if($parametros['esta']!=$datos[0]['est'])
		{
			...
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$estAnt[0]['DESCRIPCION'],$est[0]['DESCRIPCION'],$estAnt[0]['CODIGO'],$est[0]['CODIGO']);
		
		}
		if($parametros['rfid']!=$datos[0]['rfid'])
		{			
			$movimiento =' Se cambio RFID escripcion de '.$datos[0]['rfid'].' a '.$parametros['rfid'];
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['rfid'],$parametros['rfid'],'','');
		
		}
		if($parametros['tagA']!=$datos[0]['ant'])
		{
			$movimiento = 'Se cambio TAG ANTERIOR de '.$datos[0]['ant'].' a '.$parametros['tagA'];
				$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['ant'],$parametros['tagA'],'','');
		
		}
		if($parametros['seri']!=$datos[0]['SERIE'])
		{
			$movimiento =' Se cambio SERIE de '.$datos[0]['SERIE'].' a '.$parametros['seri'];
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['SERIE'],$parametros['seri'],'','');
		...
		if($datos[0]['fecha']!='')
		{
			...
			if($parametros['fech']!='' && $parametros['fech']!=$datos[0]['fecha'])
			{
				$movimiento =' Se cambio FECHA INGRESO de '.$datos[0]['fecha'].' a '.$parametros['fech'];
				$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['fecha'],$parametros['fech'],'','');
		...
		{			
			...
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['MODELO'],$parametros['mode'],'','');
		
		}
		if($parametros['obse']!=$datos[0]['OBSERVACION'])
		{
			$movimiento =' Se cambio OBSERVACION de '.$datos[0]['OBSERVACION'].' a '.$parametros['obse'];
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['OBSERVACION'],$parametros['obse'],'','');
		
		}
		if($parametros['cant']!=$datos[0]['QUANTITY'])
		{
			$movimiento=' Se cambio CANTIDAD de '.$datos[0]['QUANTITY'].' a '.$parametros['cant'];
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['QUANTITY'],$parametros['cant'],'','');
		
		}
		if($parametros['uni']!=$datos[0]['BASE_UOM'])
		{
			$movimiento=' Se cambio UNIDAD MEDIDA de '.$datos[0]['BASE_UOM'].' a '.$parametros['uni'];
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['BASE_UOM'],$parametros['uni'],'','');
		
		}
		if($datos[0]['ORIG_ACQ_YR']!='')
		{
			if(is_object($datos[0]['ORIG_ACQ_YR'])){ $datos[0]['ORIG_ACQ_YR'] = $datos[0]['ORIG_ACQ_YR']->format('Y-m-d'); }
			if($parametros['compra']!='' && $parametros['compra']!=$datos[0]['ORIG_ACQ_YR'])
			{
				$movimiento=' Se cambio FECHA DE COMPRA de '.$datos[0]['ORIG_ACQ_YR'].' a '.$parametros['compra'];
					$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['ORIG_ACQ_YR'],$parametros['compra'],'','');
		...
		if($parametros['cara']!=$datos[0]['CARACTERISTICA'])
		{
			$movimiento=' Se cambio CARACTERISTICAS de '.$datos[0]['CARACTERISTICA'].' a '.$parametros['cara'];
				$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['CARACTERISTICA'],$parametros['cara'],'','');
		
		}
		if($parametros['valor']!=$datos[0]['ORIG_VALUE'])
		{
			$movimiento=' Se cambio VALOR de '.$datos[0]['ORIG_VALUE'].' a '.$parametros['valor'];
				$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['ORIG_VALUE'],$parametros['valor'],'','');
		...
		if($parametros['act']!=$datos[0]['ORIG_ASSET'])
		{
			...
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['ORIG_ASSET'],$parametros['act'],'','');
		...
		if($bajas!=$datos[0]['BAJAS'])
		{
			...
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$dato_ant,$dato_ac,'','');
		}
		...
		if($patrimonial!=$datos[0]['PATRIMONIALES'])
		{
			...
			{
				$movimiento =' Se MARCO como PATRIMONIAL';
				$dato_ac = 1;
				$dato_ant = 0;
			}else
			{
				$movimiento =' Se DESMARCO como PATRIMONIAL';
				$dato_ac = 0;
				$dato_ant = 1;
			}
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$dato_ant,$dato_ac,'','');
		...
		{
			$op='';
			if($datos[0]['TERCEROS']=='1'){ $op='TERCEROS';}
			if($datos[0]['BAJAS']=='1'){ $op='BAJAS';}
			if($datos[0]['PATRIMONIALES']=='1'){ $op='PATRIMONIALES';}
			$movimiento=' Se CAMBIO de '.$op.' A NINGUNO';			
			//$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$op,'NINGUNO');
		}
		...
		if($parametros['crit']!=$datos[0]['idpro'])
		{
			...
			$this->registrar_movimiento($id,$movimiento,$seccion='ARTICULOS',$datos[0]['proyecto'],$pro[0]['desc'],$proAnt[0]['pro'],$pro[0]['pro']);
		}
