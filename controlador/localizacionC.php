...
if(isset($_GET['lista']))
{
	...
	echo json_encode($controlador->lista_localizacion($query,$parametros));
}
// if(isset($_GET['localizacion_custodio']))
// {
// 	$query = '';
...
// 		$query = $_GET['q'];
// 	}
// 	echo json_encode($controlador->localizacion_custodio($query,$parametros));
// }
...
class localizacionC
{
	...
	function lista_localizacion($query,$parametros)
	{
		// print_r($query);print_r($parametros);die();
		$cambio = array();
		if(isset($parametros['masivo']) && $parametros['masivo']==1)
		{
			$custodios = explode('-',$parametros['custodio']);
			$cus = '';
			foreach ($custodios as $key => $value) {
				$cus.="'".$value."',";
			}
			$custodios = substr($cus, 0,-1);

			// print_r($custodios);die();

			// print_r('gru');die();
			$lista = $this->modelo->localizacion_custodio($query,$custodios);
		}else{	
	...
	function localizacion_custodio($query,$parametros)
	{
		print_r($query);
		print_r($parametros);
		die();
